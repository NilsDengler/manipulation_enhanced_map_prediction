...
class InfoGainEval:
    def __init__(self,camera_matrices_dir,subsample = 8,width = 640,height=480,start_k = 60,max_k = 420,step =0.002,
        ...
        self.eps = 0.0000001
        grid = np.meshgrid(xrange,yrange,zrange,indexing = 'ij')
        self.max_x_bounds = grid[0].max()
        self.min_x_bounds = grid[0].min()
        self.min_y_bounds = grid[1].min()
        self.max_y_bounds = grid[1].max()
        self.max_z_bounds = grid[2].max()
        self.min_z_bounds = grid[2].min()
    ...
    def get_ray(self, camera_matrix):
        extrinsic = o3d.core.Tensor(camera_matrix['o3d_extrinsic_matrix'].reshape(4, 4, order='C'))
        intrinsic = o3d.core.Tensor(camera_matrix['intrinsic_matrix'].reshape(3, 3, order='C'))
        ray = o3d.t.geometry.RaycastingScene.create_rays_pinhole(intrinsic_matrix=intrinsic,
                                                                  extrinsic_matrix=extrinsic,
                                                                  width_px=self.width, height_px=self.height)
        return ray[::self.subsample, ::self.subsample, :].numpy().reshape((-1, 6))


    def init_camera_rays(self):
        self.all_rays = []
        for camera in range(len(self.camera_matrices)):
            self.all_rays.append(self.get_ray(self.camera_matrices[camera]))



    def get_raycast(self, camera_matrix=None, camera_idx=None, as_cupy = False):
        if(self.cached and camera_idx is not None):
            return self.all_raycasts[camera_idx]
        elif camera_idx is not None and camera_matrix is None:
            if(as_cupy):
            ...
            if(as_cupy):
                return self.get_raycast_indices(camera_matrix, use_matrix=True)
            else:
                return cp.asnumpy(self.get_raycast_indices(camera_matrix, use_matrix=True))


    def get_raycast_indices(self, camera, use_matrix=False):
        """
        ...
        """
        if use_matrix:
            ray = self.get_ray(camera)
        else:
            ray = self.all_rays[camera]

        # Separate ray origins and directions
        origins_cp = cp.asarray(ray[:, :3])
        directions_cp = cp.asarray(ray[:, 3:])
    ...
    def get_occlusion_aware_information_gain(self,raycast, voxels_of_interest,
                                             prob_bar=None, log_prob=None, log_prob_bar=None,
                                             use_beta_entropy=False, beta_entropy=None, n_cameras = None,
                                             information_voxels_of_interest = None, use_alternative=False):

            negative_indices = cp.all(raycast == 0,axis = 2)
            inv_raycast_y = voxels_of_interest.shape[1] - raycast[:, :, 0]
            readings = cp.ascontiguousarray(voxels_of_interest[raycast[:,:,1],inv_raycast_y,raycast[:,:,2]])

            if use_alternative and prob_bar is not None and log_prob is not None and log_prob_bar is not None:
    ...
    def get_ig_single_view(self, voxels_of_interest, camera, use_beta_entropy=False, beta_entropy=None, use_alternative=False, raycast = None):
        ...
        if raycast is None:
            raycast = self.get_raycast(camera_matrix=camera, as_cupy=True)
        else:
            raycast = cp.asarray(raycast)
        information_gain = self.get_occlusion_aware_information_gain(raycast, prob, prob_bar, log_prob, log_prob_bar,
                                                                     use_beta_entropy=use_beta_entropy,
                                                                     beta_entropy=beta_entropy,
                                                                     use_alternative=use_alternative)
        return cp.asnumpy(information_gain), cp.asnumpy(raycast)


    def get_all_igs(self,voxels_of_interest, use_beta_entropy=False, beta_entropy=None, use_alternative=False, skip = 1):
        igs = []
        if use_alternative:
            prob = cp.copy(voxels_of_interest)
            prob[0, 0, 0] = 0
            ...
            prob, prob_bar, log_prob, log_prob_bar = None, None, None, None

        for camera in range(0,len(self.camera_matrices),skip):
            raycast = self.get_raycast(camera_idx=camera,as_cupy=True)
            if(self.cached):
                raycast = cp.asarray(raycast)
            information_gain = self.get_occlusion_aware_information_gain(raycast, prob, prob_bar, log_prob, log_prob_bar,
                                                                         use_beta_entropy=use_beta_entropy,
    ...
    def get_relative_beta_entropy(self, beta_dist):
        """ Compute the relative entropy of a Beta distribution Beta(alpha, beta) to a uniform prior.
                Args:
        ...
        beta = beta_dist[0, ::2, :, :]
        beta_torch = torch.distributions.Beta(alpha, beta)
        uniform_beta_torch = torch.distributions.Beta(torch.ones_like(alpha), torch.ones_like(beta))
        kld = torch.distributions.kl.kl_divergence(beta_torch, uniform_beta_torch)
        return cp.asarray(kld.cpu().numpy())
    ...
    def get_all_subsequent_igs(self,voxels_of_interest, already_viewed_views, use_beta_entropy=False, beta_entropy=None):
        ...
        for view in already_viewed_views:
            raycast = self.get_raycast(camera_idx=view,as_cupy=True)
            information_voxels_of_interest[raycast[:,:,1],voxels_of_interest.shape[1]-raycast[:,:,0],raycast[:,:,2]] = 0
        igs = []
        prob_bar, log_prob, log_prob_bar = None, None, None
        for camera in range(len(self.camera_matrices)):
            raycast = self.get_raycast(camera_idx=camera,as_cupy=True)
            if(self.cached):
                raycast = cp.asarray(raycast)
            information_gain = self.get_occlusion_aware_information_gain(raycast, voxels_of_interest, prob_bar, log_prob, log_prob_bar,
                                                                         use_beta_entropy=use_beta_entropy,
                                                                         beta_entropy=beta_entropy,
    ...
    def get_all_igs_parallel(self,voxels_of_interest):
        ...
        for i in range(self.parallel_passes):
            raycast = self.raycast[i*self.parallel_passes*self.original_shape[1]:(i+1)*(self.parallel_passes)*self.original_shape[1]]
            information_gain = self.get_occlusion_aware_information_gain(raycast,
                                                                         voxels_of_interest,
                                                                         n_cameras = 300//self.parallel_passes,
                                                                         use_beta_entropy=False,
                                                                         use_alternative=False)

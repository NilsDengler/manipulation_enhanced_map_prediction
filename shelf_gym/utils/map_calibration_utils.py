...
class mECE_Calibration_calc_3D:
    ...
    def update_bins(self,semantic_label,semantic_label_gt):
        ...
        semantic_label = np.asarray(semantic_label)
        semantic_label_gt = np.asarray(semantic_label_gt)
        if(self.one_hot):
    ...
    def return_calibration_results(self):
        results = {}
        for i in range(self.n_classes):
            results.update({i:list(self.cals[i].return_calibration_results())+[self.cals[i].total_bin_members,self.cals[i].get_ECE()]})
        results.update({'aggregate':list(self.agg_cal.return_calibration_results())+[self.agg_cal.total_bin_members,self.agg_cal.get_ECE()]})
        return results
    ...
    def get_ECEs(self):
        results = []
        for i in range(self.n_classes):
            results.append(np.asnumpy(self.cals[i].get_ECE()))
        results.append(np.asnumpy(self.agg_cal.get_ECE()))
        return results
    ...
    def get_mECE(self):
        ...
        for i in range(self.n_classes):
            if(i !=0):
                ECEs.append(np.asarray(self.cals[i].get_ECE()))
        ...
        return np.asnumpy(np.mean(ECEs))
...
class Cumulative_mIoU:
    ...
    def get_IoUs(self):
        return np.asnumpy(self.intersections/self.unions)

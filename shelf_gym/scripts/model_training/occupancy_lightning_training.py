...
from matplotlib import pyplot as plt
from shelf_gym.utils.learning_utils.losses import evidential_occupancy_loss_3D,evidential_semantic_loss_3D
...
class OccupancyMapPredictor(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        gt_3d, permuted_free, permuted_occupied = self.dp.data_prep(batch)
        # pdb.set_trace()
        steps = 0
        previous_map = self.dp.get_initial_map(permuted_free)
        optim.zero_grad()
        step_losses = []
        step_accuracies = []
        for free, occupied in zip(permuted_occupied, permuted_free):
            model_input = self.dp.get_model_input(occupied, free, previous_map)
            model_output = self.predictor(model_input)
            this_loss = evidential_loss_3D(model_output, gt_3d, k=self.current_epoch / self.trainer.max_epochs)
            # print(this_loss.detach().cpu().numpy())
            steps += 1
            # this_loss.backward()
            self.manual_backward(this_loss)
            acc = map_accuracy(model_output, gt_3d).cpu().numpy()
            step_accuracies.append(acc)
            step_losses.append(this_loss.detach().cpu())
            previous_map = model_output.detach().clone()

        optim.step()
        loss = np.mean(step_losses)
        accuracy = np.mean(step_accuracies)
        self.log_dict({'training_loss': loss, 'training_accuracy': accuracy})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for free, occupied in zip(permuted_occupied, permuted_free):
            model_input = self.dp.get_model_input(occupied, free, previous_map)
            model_output = self.predictor(model_input)
            this_loss = evidential_loss_3D(model_output, gt_3d, k=1)
            # print(this_loss.detach().cpu().numpy())
            steps += 1
            acc = map_accuracy(model_output, gt_3d).cpu().numpy()
            step_accuracies.append(acc)
    ...
    def data_prep(self, data):
        ...
        return gt_3d, permuted_free, permuted_occupied

    def get_model_input(self, occupied, free, previous_map):
        # free_map
        if (self.mode == '3D_all_inputs'):
            model_input = torch.cat((previous_map, occupied, free), dim=1)
        elif (self.mode == '3D_denoising'):
            model_input = previous_map
            model_input[:, ::2, :, :] += occupied
            model_input[:, 1::2, :, :] += free
        return model_input

    def _get_initial_map(self, permuted_free):
        if ((self.mode == '3D_all_inputs') or (self.mode == '3D_denoising')):
    ...
    def test_step(self, batch, bathc_idx):
        ...
        gt_3d, permuted_free, permuted_occupied = self.dp.data_prep(data)
        # pdb.set_trace()
        steps = 0
        previous_map = self.dp.get_initial_map(permuted_free)
        step_losses = []
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(self.predictor.parameters(), lr=0.00005)


class SemanticMapPredictor(OccupancyMapPredictor):
    ...
    def get_mixing_coefficient(self):
        if (self.k_type == 'fixed'):
            ...
            new_k = min(self.global_step / self.total_steps, 1)
            return new_k
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        for occupied, free, semantics in zip(permuted_occupied, permuted_free, permuted_semantics):
            ...
            occupancy_loss = evidential_occupancy_loss_3D(occupancy_beta, gt_3d, k=this_k)
            semantic_loss = evidential_semantic_loss_3D(semantic_map, gt_semantics, k=this_k)
            sem_losses.append(semantic_loss.detach().cpu().numpy().tolist())
            occ_losses.append(occupancy_loss.detach().cpu().numpy().tolist())

            this_loss = occupancy_loss + semantic_loss
            # print(this_loss.detach().cpu().numpy())
            self.manual_backward(this_loss)
            ...
            if (self.update_frequency == 'every'):
                self.clip_gradients(optim, gradient_clip_val=2, gradient_clip_algorithm='norm')
        ...
        with torch.no_grad():
            ...
            for occupied, free, semantics in zip(permuted_occupied, permuted_free, permuted_semantics):
                ...
                if (self.k_type == 'fixed'):
                    ...
                    k = 1
                occupancy_loss = evidential_occupancy_loss_3D(occupancy_beta, gt_3d, k=k)
                semantic_loss = evidential_semantic_loss_3D(semantic_map, gt_semantics, k=k)
                sem_losses.append(semantic_loss.detach().cpu().numpy().tolist())
                occ_losses.append(occupancy_loss.detach().cpu().numpy().tolist())
                this_loss = occupancy_loss + semantic_loss
        ...
        with torch.no_grad():
            ...
            for occupied, free, semantics in zip(permuted_occupied, permuted_free, permuted_semantics):
                ...
                if(normalize):
                    ...
                    occupancy_probs = torch.clone(occupancy_beta)
                    semantic_probs = torch.clone(semantic_map)
                if(intermediates):
...
if (__name__ == '__main__'):
    ...
    parser.add_argument('--mode', default = '3D_denoising')
    ...
    # dataset = MapDataset(dataset_dir,max_samples = 10)
    dataset_dir = './large_map_completion_fixed.hdf5'
    # dataset_dir = '/tmp/large_map_completion_fixed.hdf5'
    # dataset_dir = '/scratch/bdds/jcorreiamarques/map_completion_fixed.hdf5'
    # val_dir = './map_completion_fixed.hdf5'
    # dataset_dir = './map_completion.hdf5'
    dataset = MapDatasetH5py(dataset_dir, max_samples=10, skip=skip)
    ...
    # k_type = 'variable'
    n_classes = 7
    wandb_logger = WandbLogger(name = 'Non_Destroying_Normalization_{}_{}_{}_recursive_{}_Normalized_{}_max_alpha_{}'.format(
        mode,k_type,update_frequency,recursive,normalize,max_alpha).replace('.','_'),
        project = 'Semantic Map Completion Final',
        log_model ='all',save_dir = './wandb_logs')

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    if (mode == '3D_all_inputs'):
        underlying_model = SemanticUNet(408, 204, n_semantic_channel_in=2 * n_classes,
                                        do_dropout=do_dropout,max_alpha = max_alpha,normalize = normalize).to(device)
    elif (mode == '3D_semantic_augmented_all_inputs'):
        underlying_model = SemanticUNet(408+2*n_classes, 204,
                                        n_semantic_channel_in=2 * n_classes, do_dropout=do_dropout,
                                        max_alpha = max_alpha,normalize = normalize).to(device)
        ...
        raise NotImplementedError()
    model = SemanticMapPredictor(predictor=underlying_model, mode=mode, lr=0.00001, k_type=k_type, k=k,
                                 total_steps=total_steps, update_frequency=update_frequency, recursive=recursive,normalize = normalize,max_alpha=max_alpha)

    # compiled_model = torch.compile(model,mode='reduce-overhead')
    # print('\n\ncompiled model!\n\n')
    # prof = SimpleProfiler(dirpath= './debugging_logs/',filename = 'debug_log2.txt')
    prof = None
    early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=0.00, patience=12, verbose=False, mode="min")
    checkpoint_callback = ModelCheckpoint(monitor="val_loss", mode="min", every_n_train_steps=val_check_interval + 1)
    trainer = L.Trainer(precision="32-true", enable_progress_bar=True,
                        ...
                        num_sanity_val_steps=0, use_distributed_sampler=True)
